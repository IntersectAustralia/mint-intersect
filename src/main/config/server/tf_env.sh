...
export NLA_ENABLED="${nla.enabled}"
...
# additional settings
EXTRA_OPTS="-Dserver.url.base=$SERVER_URL -Dserver.ip=$SERVER_IP -Damq.port=$AMQ_PORT -Damq.stomp.port=$AMQ_STOMP_PORT -Dsmtp.host=$SMTP_HOST -Dadmin.email=$ADMIN_EMAIL -Dredbox.version=$REDBOX_VERSION -Dnla.enabled=$NLA_ENABLED"
